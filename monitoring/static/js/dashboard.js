(function () {
  ...
  function plotlyTemplate() {
    return isDarkMode() ? "plotly_dark" : "plotly_dark";
  }
  ...
  function initDataTables() {
    ...
    if (qs("price-table") && !window.jQuery.fn.DataTable.isDataTable("#price-table")) {
      priceTableInstance = window.jQuery("#price-table").DataTable({
        pageLength: 10,
        order: [[2, "desc"]],
        responsive: true,
      });
    ...
    if (qs("ohlcTable") && !window.jQuery.fn.DataTable.isDataTable("#ohlcTable")) {
      ohlcTableInstance = window.jQuery("#ohlcTable").DataTable({
        pageLength: 10,
        order: [[1, "desc"]],
        responsive: true,
      });
  ...
  function renderLivePriceTable(snapshots) {
    ...
    if (!snapshots.length) {
      tbody.innerHTML = `
        <tr>
          <td colspan="3" class="text-center text-muted">No price snapshot data available.</td>
        </tr>
      `;
      return;
    }
  ...
  function renderOhlcTable(data, symbol) {
    ...
    if (ohlcTableInstance) {
      ...
      if (!rows.length) {
        ohlcTableInstance.row.add([symbol || "—", "—", "—", "—", "—", "—", "—", "—"]);
    ...
    if (!rows.length) {
      tbody.innerHTML = `
        <tr>
          <td colspan="8" class="text-center text-muted">No OHLC data available.</td>
        </tr>
      `;
      return;
    }
  ...
  async function loadLinePriceHistoryChart(symbol, start = "", end = "") {
    ...
    if (!hasPlotly()) {
      showEmptyState(chartId, "Plotly is not available.");
      return;
    }
    ...
    try {
      ...
      if (!rows.length) {
        showEmptyState(chartId, "No valid OHLC history data.");
      ...
      showEmptyState(chartId, "Unable to load line-price history.");
  ...
  async function loadCandlestickChart(symbol, start = "", end = "") {
    ...
    try {
      ...
      if (!ohlc.length) {
        showEmptyState(chartId, "No OHLC data available.");
      ...
      showEmptyState(chartId, "Unable to load candlestick data.");
  ...
  async function refreshDashboard() {
    await fetchSnapshotsAndRefreshTable();
    await refreshOhlcTable();
    await pollAlerts();
    ...
    if (lineSymbol) {
      await loadLinePriceHistoryChart(
        lineSymbol,
        qs("lineStartDate")?.value || "",
        qs("lineEndDate")?.value || ""
      );
  ...
  function initDashboard() {
    const dashboardRoot = document.querySelector(".mv-dashboard");
